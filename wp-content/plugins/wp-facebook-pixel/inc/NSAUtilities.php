...
if(!function_exists('nsau_GetMetaKeys')) {
    /**
     * Gets the name of all meta keys used for all post types.
...
if ( ! function_exists('nsau_Write_to_Log')) {
    ...
    function nsau_Write_to_Log ($title, $log)  {
        ...
        if(is_array($caller)) {
            error_log(sprintf("nsau_Write_to_Log: %s - %s%s%s@%s(%s)", $title, $caller['class'], $caller['type'], $caller['function'], $caller['line'], print_r($caller['args'], true)));
