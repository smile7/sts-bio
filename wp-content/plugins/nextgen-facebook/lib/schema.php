	...
	class NgfbSchema {
		...
		public static function add_single_organization_data( &$json_data, $mod, $org_id = 'site', $logo_key = 'org_logo_url', $list_element = false ) {
			...
			if ( isset( $cache[$mod['name']][$mod['id'][$org_id]] ) ) {
				if ( $ngfb->debug->enabled ) {
					$ngfb->debug->log( 'using static cache data for organization id '.$org_id );
				}
			...
			if ( $ngfb->debug->enabled ) {
				$ngfb->debug->log( 'creating static cache data for organization id '.$org_id );
			}
		...
		public static function add_single_place_data( &$json_data, $mod, $place_id = false, $list_element = false ) {
			...
			if ( isset( $cache[$mod['name']][$mod['id'][$place_id]] ) ) {
				if ( $ngfb->debug->enabled ) {
					$ngfb->debug->log( 'using static cache data for place id '.$place_id );
				}
			...
			if ( $ngfb->debug->enabled ) {
				$ngfb->debug->log( 'creating static cache data for place id '.$place_id );
			}
		...
		public static function add_single_event_data( &$json_data, $mod, $event_id = false, $list_element = false ) {
			...
			if ( isset( $cache[$mod['name']][$mod['id'][$event_id]] ) ) {
				if ( $ngfb->debug->enabled ) {
					$ngfb->debug->log( 'using static cache data for event id '.$event_id );
				}
			...
			if ( $ngfb->debug->enabled ) {
				$ngfb->debug->log( 'creating static cache data for event id '.$event_id );
			}
		...
		public static function add_single_person_data( &$json_data, $mod, $user_id, $list_element = true ) {
			...
			if ( isset( $cache[$mod['name']][$mod['id'][$user_id]] ) ) {
				if ( $ngfb->debug->enabled ) {
					$ngfb->debug->log( 'using static cache data for person id '.$user_id );
				}
			...
			if ( $ngfb->debug->enabled ) {
				$ngfb->debug->log( 'creating static cache data for person id '.$user_id );
			}
		...
		public static function add_single_image_data( &$json_data, $opts, $prefix = 'og:image', $list_element = true ) {
			...
			if ( ! empty( $opts[$prefix.':id'] ) && is_numeric( $opts[$prefix.':id'] ) ) {
				...
				$ret['description'] = $ngfb->page->get_description( $ngfb->options['schema_desc_len'],
					'...', $mod, true, false, true, 'schema_desc' );
