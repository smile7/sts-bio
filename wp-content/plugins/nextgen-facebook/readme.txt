...
* *Developer Notes*
	* Added a static cache in the NgfbSchema add_single_organization_data(), add_single_place_data(), add_single_event_data(), and add_single_person_data() methods.
	* Added new custom field options under the NGFB &gt; Advanced &gt; Custom Meta tab:
		* Additional Type URLs Custom Field
		* Product Brand Custom Field
		* Product Color Custom Field
		* Product Size Custom Field

**Version 8.45.5 (2017/08/08)**
...
* *Developer Notes*
	...
	* Added a new 'ngfb_sharing_buttons_shared_url' filter.
...
(2017/08/23) Added support for custom Schema Event start / end date and time values. Added a test for WooCommerce v3.0.0 product 'get_date_on_sale_from' and 'get_date_on_sale_to' methods before calling them (Pro version). Added new custom field options under the NGFB &gt; Advanced &gt; Custom Meta tab (Pro version).

= 8.45.5 =

(2017/08/08) Minor update to "itemscope" string matching in the head attributes. Added https://schema.org/Intangible and https://schema.org/Accommodation (and their sub-types) to the selection of available Schema types. Added a new 'ngfb_sharing_buttons_shared_url' filter.
